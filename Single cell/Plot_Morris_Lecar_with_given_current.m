function Plot_Morris_Lecar_with_given_current(I_span, tau, time, initPos1)
...
Folder = ['image\tau_' num2str(tau) '_I_max\'];
if ~exist(Folder, 'dir')
    mkdir(Folder);
end
...
for i = 1:length(I_span)
    ...
    title(['I_{max} = ' num2str(I_span(i)) '  tau = ' num2str(tau)], 'Fontsize', 12);
    
    print(h, [Folder 'I_max_' num2str(I_span(i)) '.jpg'], '-djpeg', '-r300');
    close(h);
end
MakeCurrentVideo(I_span, tau, freq)
time = toc;
